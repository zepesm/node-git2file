# Repository to Single File (`git2file.js`)
...
1.  Clone the repository or download the `git2file.js` and `package.json` files.
...
To make the script available as a command (`git2file`) callable from any directory:
...
3.  After successful installation, you can run the script from anywhere using the `git2file` command instead of `node git2file.js`.
...
```bash
node git2file.js <start_path> <output_file> [custom_exclusion_file] [.ext1 .ext2 ...]
```

**If installed globally (using `npm install -g .`):**

```bash
git2file <start_path> <output_file> [custom_exclusion_file] [.ext1 .ext2 ...]
```
